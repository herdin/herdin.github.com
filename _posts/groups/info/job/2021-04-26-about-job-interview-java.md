...
- design pattern
  ...
  - method chain
  - template
  - strategy -> 동작(전략)을 외부에서 interface 구현체로 받아 사용
  - command -> command interface 구현체에 따라 내부 객체의 어떤 동작을 수행 할지를 결정
- junit
  - 3, 4, 5 life cycle
  - 차이점?
...
이를 위해 Young/Old 영역으로 물리적 공간을 나눔. 새로 만들어진 객체는 Yong Generation -> 오래되면 Old Generation(Young < Old), Permanent 영역은 메소드공간
...
- Collection interface, 순서나 집합
  ...
  2. List interface, 순서O 중복O
    2.1. Linked List
    2.2. Vector, ArrayList + ThreadSafe
      2.2.1. Stack(Vector)
    2.3. ArrayList
  3. Set interface, 순서X 중복X
    3.1. HashSet
    3.2. SortedSet interface, 순서O
      - TreeSet
- Map interface, 키-값, 순서X 키중복X 값중복O
  1. HashTable, ThreadSafe
  2. HashMap
  3. SortedMap interface, 순서O
    - TreeMap
...
- 병렬처리에 신경쓰지 않아도 된다.
- 파이프라이닝으로 처리하기 떄문에 laziness(요청할때 계산), short-circuiting(allMatch, noneMatch, findFirst, findAny, 전체를 계산하지 않아도 결과를 낼 수 있음) 같은 최적화를 할 수 있다.

#### singletone 을 사용할 때 생기는 문제와 해결 방안에 대해서 말해보세요.
